...
public class UnarchiveJobApplicationEndpoint : EndpointBaseAsync.WithRequest<JobApplicationIdRequestDTO>.WithResult<IActionResult>
{
    private readonly ISender _sender;

    public UnarchiveJobApplicationEndpoint(ISender sender)
    {
        _sender = sender;
    }

    [Authorize(AuthenticationSchemes = "BearerEmployee,BearerCandidate", Policy = Policies.CandidateOnly)]
    [HttpPost("job-applications/{JobApplicationId}/unarchive")]
    ...
    {
        var result = await _sender.Send(new SetJobApplicationArchivedCommand(request.JobApplicationId, archived: false), cancellationToken);
