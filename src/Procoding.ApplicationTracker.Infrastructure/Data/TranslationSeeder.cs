...
public static class TranslationSeeder
{
    private static readonly (string Key, string Hr, string En)[] BaseTranslations =
    {
        ...
        // ---- Auth pages (MVC login / signup / admin) ----
        ("auth.field.email", "Email", "Email"),
        ("auth.field.password", "Lozinka", "Password"),
        ("auth.field.firstName", "Ime", "First name"),
        ("auth.field.lastName", "Prezime", "Last name"),
        ("auth.point.pipeline", "Vidi cijeli tijek na jednoj ploči", "See your whole pipeline on one board"),
        ("auth.point.neverLose", "Nijedna prijava ti neće promaknuti", "Never lose track of an application"),
        ("auth.point.whatsNext", "Uvijek znaj što točno slijedi", "Always know exactly what's next"),
        ("auth.point.freeStart", "Besplatno za početak — bez kreditne kartice", "Free to start — no credit card required"),
        ("auth.point.trackAll", "Prati prijave, intervjue i ponude", "Track applications, interviews and offers"),
        ("auth.point.pipelineGlance", "Vidi cijeli tijek u jednom pogledu", "See your whole pipeline at a glance"),

        ("auth.login.title", "Prijava", "Sign in"),
        ("auth.login.welcomeBack", "Dobro došao natrag.", "Welcome back."),
        ("auth.login.welcomeSub", "Nastavi točno ondje gdje si stao i drži svoju potragu za poslom u pokretu.", "Pick up right where you left off and keep your job search moving."),
        ("auth.login.enterDetails", "Unesi svoje podatke za pristup ploči.", "Enter your details to access your board."),
        ("auth.login.accountCreated", "Račun je kreiran — prijavi se.", "Account created — please sign in."),
        ("auth.login.submit", "Prijavi se", "Sign in"),
        ("auth.login.noAccount", "Nemaš račun?", "Don't have an account?"),
        ("auth.login.createFree", "Kreiraj ga besplatno", "Create one for free"),
        ("auth.login.staff", "Zaposlenik?", "Staff member?"),
        ("auth.login.adminSignin", "Prijava za administratore", "Admin sign in"),

        ("auth.signup.title", "Kreiraj račun", "Create your account"),
        ("auth.signup.takeControl", "Preuzmi kontrolu nad potragom za poslom.", "Take control of your job search."),
        ("auth.signup.takeControlSub", "Kreiraj besplatni račun i pretvori neurednu tablicu u jasan proces koji vodi do cilja.", "Create your free account and turn a messy spreadsheet into a clear, winnable process."),
        ("auth.signup.startTracking", "Počni pratiti svoje prijave u nekoliko minuta.", "Start tracking your applications in minutes."),
        ("auth.signup.createPasswordPh", "Kreiraj lozinku", "Create a password"),
        ("auth.signup.submit", "Kreiraj besplatni račun", "Create free account"),
        ("auth.signup.haveAccount", "Već imaš račun?", "Already have an account?"),
        ("auth.signup.signin", "Prijavi se", "Sign in"),

        ("auth.admin.title", "Prijava za administratore", "Admin sign in"),
        ("auth.admin.console", "Administratorska konzola.", "Admin console."),
        ("auth.admin.consoleSub", "Upravljaj kandidatima, tvrtkama, izvorima i prijevodima aplikacije.", "Manage candidates, companies, sources and the app's translations."),
        ("auth.admin.point1", "Potpun pregled svih prijava", "Full view across all applications"),
        ("auth.admin.point2", "Uređuj hrvatski i engleski tekst sučelja", "Edit the Croatian & English UI text"),
        ("auth.admin.point3", "Održavaj podatke platforme urednima", "Keep the platform data tidy"),
        ("auth.admin.forStaff", "Za interno osoblje i administratore.", "For internal staff and administrators."),
        ("auth.admin.submit", "Prijavi se kao administrator", "Sign in as admin"),
        ("auth.admin.notAdmin", "Nisi administrator?", "Not an admin?"),
        ("auth.admin.candidateSignin", "Prijava za kandidate", "Candidate sign in"),
